...
import { HORIZON_DAYS, INITIAL_BARS, INITIAL_CASH, TOTAL_BARS, clamp, type ReplayAction } from "./game-config";
...
type ChallengeBundle = { date: string; stock: StockSample };
...
export default function GameClient({ initialChallenge }: { initialChallenge: ChallengeBundle }) {
  ...
  const resetGame = async (nextMode: GameMode) => {
    ...
    try {
      let nextStock = initialChallenge.stock;
      if (nextMode === "practice") {
        const seed = crypto.randomUUID();
        const response = await fetch(`/api/challenge?seed=${encodeURIComponent(seed)}`);
        if (!response.ok) throw new Error("challenge load failed");
        nextStock = ((await response.json()) as ChallengeBundle).stock;
      }
      submissionRef.current = false; setGameMode(nextMode); setStock(nextStock); setVisibleCount(INITIAL_BARS); setRound(0); setCash(INITIAL_CASH); setShares(0); setMode("buy"); setAllocation(1); setRevealDays(3); setTrades(0); setTradeMarkers([]); setEquityHistory([INITIAL_CASH]); setActions([]); setFinished(false); setResultOpen(false); setRevealPulse(0); setShareStatus(""); setScoreStatus("idle");
    } finally {
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <div className="chart-panel">{revealPulse > 0 && <span key={revealPulse} className="reveal-toast">+{actions.at(-1)?.days || revealDays} 个交易日</span>}<div className="chart-head"><div><span className="ticker-mask">••••••</span><span className="market-tag">日 K</span><span className="adjust-tag">归一化 · 复权</span></div><div className="ohlc"><span>开 <b>{current.open.toFixed(2)}</b></span><span>高 <b>{current.high.toFixed(2)}</b></span><span>低 <b>{current.low.toFixed(2)}</b></span><span>收 <b>{current.close.toFixed(2)}</b></span><strong className={dayChange >= 0 ? "up" : "down"}>{dayChange >= 0 ? "+" : ""}{dayChange.toFixed(2)}%</strong></div></div><div className="ma-row"><span>MA5 {ma5?.toFixed(2)}</span><span>MA10 {ma10?.toFixed(2)}</span><span>MA20 {ma20?.toFixed(2)}</span><em>相对量 {current.volume ? (current.volume / (data.slice(-20).reduce((sum, candle) => sum + candle.volume, 0) / Math.min(20, data.length))).toFixed(2) : "0"}×</em></div><CandleChart data={data} markers={tradeMarkers} /></div>
    ...
    </section><footer className="source-note">12 只 A 股上市以来真实连续复权日线 · 覆盖 1991–2026 多轮牛熊周期 · 不构成投资建议</footer>

    {rulesOpen && <dialog open className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setRulesOpen(false); }}><section className="rules-modal"><button className="modal-close" onClick={() => setRulesOpen(false)}>×</button><small>HOW TO PLAY</small><h2>在真实牛熊周期里做决策</h2><ol><li><b>观察</b><span>开局提供 120 根真实日 K，覆盖约半年走势，股票身份和历史日期隐藏。</span></li><li><b>决策</b><span>买入、卖出或空仓，委托统一在下一交易日开盘成交并留下 B/S 点。</span></li><li><b>持有</b><span>每次选择向前推进 1、3 或 5 个真实交易日，期间仓位不变、净值逐日计算。</span></li><li><b>周期</b><span>每局最多推进 60 个交易日，行情来自 12 只股票上市以来的真实连续复权日线。</span></li><li><b>评分</b><span>综合超额收益、最大回撤和与持有周期匹配的交易纪律。</span></li><li><b>复盘</b><span>结算后揭晓真实股票、日期、交易点和本局画像。</span></li></ol><button className="primary-action" onClick={() => setRulesOpen(false)}>继续挑战</button></section></dialog>}
