...
import { INITIAL_BARS, INITIAL_CASH, ORDER_ALLOCATIONS, clamp, initialBarsFor, lotSizeFor, orderQuantity, type MarketKind, type OrderAllocation, type ReplayAction } from "./game-config";
...
type TradeMarker = { index: number; type: "B" | "S"; price: number; quantity: number; round: number };
...
type ChallengeBundle = { date: string; market: MarketKind; stock: StockSample; universeSize: number; dataSource: "live-universe" | "embedded-fallback" };
...
function CandleChart({ data, markers, market }: { data: Candle[]; markers: TradeMarker[]; market: MarketKind }) {
  ...
  const dragRef = useRef<{ pointerId: number; startX: number; startOffset: number; slot: number } | null>(null);
  ...
  const maxView = data.length, effectiveView = Math.min(viewSize, data.length);
  ...
  const changeZoom = (delta: number) => {
    ...
    setViewSize((value) => {
      ...
      setRightOffset((offset) => Math.min(offset, Math.max(0, data.length - next)));
      return next;
    });
  };
  const panBy = (bars: number) => { setHover(null); setRightOffset((value) => clamp(value + bars, 0, maxOffset)); };
  const resetView = () => { setHover(null); setViewSize(Math.min(INITIAL_BARS, maxView)); setRightOffset(0); };
  const showAll = () => { setHover(null); setViewSize(maxView); setRightOffset(0); };

  useEffect(() => {
    const added = data.length - previousLengthRef.current;
    previousLengthRef.current = data.length;
    if (added > 0) setRightOffset((value) => value > 0 ? Math.min(value + added, Math.max(0, data.length - effectiveView)) : 0);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const start = viewStart, shown = data.slice(viewStart, viewEnd);
      ...
      markers.filter((marker) => marker.index >= start && marker.index < viewEnd).forEach((marker) => {
  ...
  return <div className="chart-area">
    <div className="chart-tools"><div className="trade-legend"><span className="buy-dot">B</span>买入点<span className="sell-dot">S</span>卖出点</div><div className="chart-actions"><div className="pan-tools"><button onClick={() => panBy(navigationStep)} disabled={effectiveOffset >= maxOffset} aria-label="向左查看更早K线" title="向左查看更早K线">←</button><button onClick={() => panBy(-navigationStep)} disabled={effectiveOffset <= 0} aria-label="向右查看更新K线" title="向右查看更新K线">→</button><button className="text-tool" onClick={() => setRightOffset(0)} disabled={effectiveOffset <= 0}>最新</button><input className="history-slider" type="range" min={0} max={maxOffset} value={maxOffset - effectiveOffset} disabled={maxOffset <= 0} aria-label="定位K线历史位置" onChange={(event) => { setHover(null); setRightOffset(maxOffset - Number(event.target.value)); }} /></div><div className="zoom-tools"><small>{effectiveOffset ? `距最新 ${effectiveOffset} 根` : `显示 ${effectiveView} 根`}</small><button onClick={() => changeZoom(12)} disabled={effectiveView >= maxView} aria-label="缩小K线图">−</button><button onClick={() => changeZoom(-12)} disabled={effectiveView <= 24} aria-label="放大K线图">＋</button><button className="reset-zoom" onClick={showAll} disabled={effectiveView >= maxView && effectiveOffset === 0}>全部</button><button className="reset-zoom" onClick={resetView} disabled={effectiveView === Math.min(INITIAL_BARS, maxView) && effectiveOffset === 0}>复位</button></div></div></div>
    <canvas ref={canvasRef} tabIndex={0} className={`chart-canvas ${dragging ? "dragging" : ""}`} aria-label="可缩放、拖动和键盘操作的真实历史日K线图" onDoubleClick={resetView} onWheel={(event) => { event.preventDefault(); if (event.shiftKey || Math.abs(event.deltaX) > Math.abs(event.deltaY)) panBy((event.deltaX || event.deltaY) > 0 ? navigationStep : -navigationStep); else changeZoom(event.deltaY > 0 ? 8 : -8); }} onKeyDown={(event) => { if (event.key === "ArrowLeft") panBy(navigationStep); else if (event.key === "ArrowRight") panBy(-navigationStep); else if (event.key === "Home") setRightOffset(maxOffset); else if (event.key === "End") setRightOffset(0); else if (event.key === "+" || event.key === "=") changeZoom(-12); else if (event.key === "-" || event.key === "_") changeZoom(12); else if (event.key === "0") resetView(); else return; event.preventDefault(); }} onPointerDown={(event) => { if (event.button !== 0) return; const rect = event.currentTarget.getBoundingClientRect(); event.currentTarget.setPointerCapture(event.pointerId); if (event.pointerType === "touch") { touchPointsRef.current.set(event.pointerId, { x: event.clientX, y: event.clientY }); const points = [...touchPointsRef.current.values()]; if (points.length >= 2) { pinchRef.current = { distance: Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y), viewSize: effectiveView }; dragRef.current = null; } else dragRef.current = { pointerId: event.pointerId, startX: event.clientX, startOffset: effectiveOffset, slot: Math.max(1, (rect.width - 62) / effectiveView) }; } else dragRef.current = { pointerId: event.pointerId, startX: event.clientX, startOffset: effectiveOffset, slot: Math.max(1, (rect.width - 62) / effectiveView) }; setDragging(true); setHover(null); }} onPointerUp={(event) => { touchPointsRef.current.delete(event.pointerId); pinchRef.current = null; if (dragRef.current?.pointerId === event.pointerId) dragRef.current = null; if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); setDragging(false); }} onPointerCancel={() => { touchPointsRef.current.clear(); pinchRef.current = null; dragRef.current = null; setDragging(false); }} onPointerLeave={() => { if (!dragRef.current) setHover(null); }} onPointerMove={(event) => { if (event.pointerType === "touch" && touchPointsRef.current.has(event.pointerId)) { touchPointsRef.current.set(event.pointerId, { x: event.clientX, y: event.clientY }); const points = [...touchPointsRef.current.values()], pinch = pinchRef.current; if (pinch && points.length >= 2) { const distance = Math.max(1, Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y)); const next = clamp(Math.round(pinch.viewSize * pinch.distance / distance), 24, maxView); setViewSize(next); setRightOffset((offset) => Math.min(offset, Math.max(0, data.length - next))); return; } } const drag = dragRef.current; if (drag?.pointerId === event.pointerId) { setRightOffset(clamp(drag.startOffset + Math.round((event.clientX - drag.startX) / drag.slot), 0, maxOffset)); return; } if (event.pointerType !== "mouse") return; const rect = event.currentTarget.getBoundingClientRect(); const slot = (rect.width - 62) / effectiveView; setHover({ index: Math.max(0, Math.min(effectiveView - 1, Math.floor(event.nativeEvent.offsetX / slot))), x: event.nativeEvent.offsetX, width: rect.width }); }} />
...
export default function GameClient({ initialChallenges }: { initialChallenges: InitialChallenges }) {
  ...
  const [visibleCount, setVisibleCount] = useState(() => initialBarsFor(initialChallenges.cn.stock)), [round, setRound] = useState(0);
  const [cash, setCash] = useState(INITIAL_CASH), [shares, setShares] = useState(0);
  const [mode, setMode] = useState<TradeMode>("buy"), [allocation, setAllocation] = useState<OrderAllocation>(1);
  const [orderInputMode, setOrderInputMode] = useState<OrderInputMode>("allocation"), [quantityInput, setQuantityInput] = useState("");
  ...
  const normalized = useMemo(() => { const factor = 100 / stock.candles[initialVisibleCount - 1].close; return stock.candles.map((candle) => ({ ...candle, open: candle.open * factor, close: candle.close * factor, high: candle.high * factor, low: candle.low * factor })); }, [initialVisibleCount, stock]);
  ...
  const lotSize = lotSizeFor(market), enteredQuantity = quantityInput === "" ? undefined : Number(quantityInput);
  const maxQuotedQuantity = orderQuantity({ market, kind: mode, price: current.close, cash, shares, allocation: 1 });
  const quantityError = orderInputMode !== "quantity" ? "" : enteredQuantity === undefined ? "请输入委托股数" : !Number.isInteger(enteredQuantity) || enteredQuantity <= 0 ? "请输入正整数股数" : market === "cn" && enteredQuantity % lotSize !== 0 ? `A股${mode === "buy" ? "买入" : "卖出"}须为 ${lotSize} 股的整数倍` : enteredQuantity > maxQuotedQuantity ? mode === "buy" ? "委托股数超过当前可用资金" : "委托股数超过当前持仓" : "";
  const estimatedQuantity = quantityError ? 0 : orderQuantity({ market, kind: mode, price: current.close, cash, shares, allocation, quantity: orderInputMode === "quantity" ? enteredQuantity : undefined });
  const estimatedOrderValue = estimatedQuantity * current.close;
  const advancedDays = visibleCount - initialVisibleCount;
  const availableDays = Math.max(0, normalized.length - initialVisibleCount), remainingDays = Math.max(0, availableDays - advancedDays);
  const ma5 = average(data, data.length - 1, 5), ma10 = average(data, data.length - 1, 10), ma20 = average(data, data.length - 1, 20);
  const benchmark = (current.close / data[initialVisibleCount - 1].close - 1) * 100, excess = returnRate - benchmark;
  ...
  const deepProfile = useMemo(() => buildTradeAnalysis({ candles: normalized.slice(0, visibleCount), markers: tradeMarkers, equityHistory, exposureHistory, advancedDays, returnRate, benchmark, excess, maxDrawdown }), [advancedDays, benchmark, equityHistory, excess, exposureHistory, maxDrawdown, normalized, returnRate, tradeMarkers, visibleCount]);
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      if (requestedMarket === "us") {
        ...
        setVisibleCount(initialBarsFor(initialChallenges[requestedMarket].stock));
      }
      setPlayerId(id);
  ...
  const resetSession = (nextMode: GameMode, nextStock: StockSample) => {
    submissionRef.current = false; setGameMode(nextMode); setStock(nextStock); setVisibleCount(initialBarsFor(nextStock)); setRound(0); setCash(INITIAL_CASH); setShares(0); setMode("buy"); setAllocation(1); setOrderInputMode("allocation"); setQuantityInput(""); setRevealDays(3); setTrades(0); setTradeMarkers([]); setEquityHistory([INITIAL_CASH]); setExposureHistory([0]); setActions([]); setFinished(false); setResultOpen(false); setAnalysisOpen(false); setRevealPulse(0); setShareStatus(""); setScoreStatus("idle"); setScoreboard(null);
  };
  ...
  const advance = async (action: "trade" | "hold") => {
    ...
    const requestedQuantity = orderInputMode === "quantity" ? enteredQuantity : undefined;
    const replayAction: ReplayAction = action === "hold" ? { kind: "hold", days: holdingDays as ReplayAction["days"] } : { kind: mode, ...(requestedQuantity !== undefined ? { quantity: requestedQuantity } : { allocation }), days: holdingDays as ReplayAction["days"] };
    setActions((value) => [...value, replayAction]);
    const executionIndex = visibleCount, execution = normalized[executionIndex].open;
    let nextCash = cash, nextShares = shares, didTrade = false, amount = 0;
    if (action === "trade") {
      amount = orderQuantity({ market, kind: mode, price: execution, cash, shares, allocation, quantity: requestedQuantity });
      if (mode === "buy" && amount > 0) { const spend = amount * execution; nextCash -= spend; nextShares += amount; didTrade = true; }
      if (mode === "sell" && amount > 0) { nextShares -= amount; nextCash += amount * execution; didTrade = true; }
      setOrderInputMode("allocation"); setQuantityInput("");
    }
    if (didTrade) setTradeMarkers((value) => [...value, { index: executionIndex, type: mode === "buy" ? "B" : "S", price: execution, quantity: amount, round }]);
    setCash(nextCash); setShares(nextShares);
    const pathEquities: number[] = [], pathExposures: number[] = [];
    for (let step = 1; step <= holdingDays; step++) { await delay(step === 1 ? 170 : 260); const stepPrice = normalized[visibleCount + step - 1].close; const stepEquity = nextCash + nextShares * stepPrice; pathEquities.push(stepEquity); pathExposures.push(stepEquity > 0 ? nextShares * stepPrice / stepEquity * 100 : 0); setVisibleCount(visibleCount + step); }
    const nextEquity = pathEquities[pathEquities.length - 1], nextRound = round + 1;
    setRound(nextRound); setTrades((value) => value + (didTrade ? 1 : 0)); setEquityHistory((value) => [...value, ...pathEquities]); setExposureHistory((value) => [...value, ...pathExposures]); setRevealPulse((value) => value + 1); setIsRevealing(false);
    if (advancedDays + holdingDays >= availableDays || nextEquity <= INITIAL_CASH * .2) { setFinished(true); setResultOpen(true); }
  };
  ...
  const adjustQuantity = (direction: -1 | 1) => {
    ...
    const nextQuantity = Math.max(0, Math.min(maxQuotedQuantity, currentQuantity + direction * lotSize));
    setOrderInputMode("quantity");
    setQuantityInput(nextQuantity ? String(nextQuantity) : "");
  };

  const tradeDisabled = isRevealing || finished || remainingDays <= 0 || estimatedQuantity <= 0 || Boolean(quantityError);
  return <main className="shell">
    <header className="topbar"><div className="brand"><span className="brand-mark">K</span><span>盲盘</span></div><div className="market-switch" role="group" aria-label="选择股票市场"><button className={market === "cn" ? "active" : ""} disabled={challengeLoading || isRevealing} onClick={() => changeMarket("cn")}>A股</button><button className={market === "us" ? "active" : ""} disabled={challengeLoading || isRevealing} onClick={() => changeMarket("us")}>美股</button></div><div className="round-pill"><span>{gameMode === "daily" ? `${marketLabel}今日盲盘` : `${marketLabel}练习`}</span><i />已推进 {advancedDays} 个交易日</div><div className="top-actions"><button className="player-chip" onClick={() => setScoreboardOpen(true)}><i>{nickname.slice(0, 1)}</i><span>{nickname}</span>{scoreboard?.stats?.streak ? <b>🔥 {scoreboard.stats.streak}</b> : null}</button><button className="text-button rank-button" onClick={() => setScoreboardOpen(true)}>今日排行</button><button className="text-button" onClick={() => setRulesOpen(true)}>游戏规则</button></div></header>
    ...
    <section className="workspace">
      <div className="chart-panel">{revealPulse > 0 && <span key={revealPulse} className="reveal-toast">+{actions.at(-1)?.days || revealDays} 个交易日</span>}<div className="chart-head"><div><span className="ticker-mask">••••••</span><span className="market-tag">{marketLabel}</span><span className="market-tag">日 K</span><span className="adjust-tag">归一化 · 复权</span></div><div className="ohlc"><span>开 <b>{current.open.toFixed(2)}</b></span><span>高 <b>{current.high.toFixed(2)}</b></span><span>低 <b>{current.low.toFixed(2)}</b></span><span>收 <b>{current.close.toFixed(2)}</b></span><strong className={dayChange >= 0 ? "up" : "down"}>{dayChange >= 0 ? "+" : ""}{dayChange.toFixed(2)}%</strong></div></div><div className="ma-row"><span>MA5 {ma5?.toFixed(2)}</span><span>MA10 {ma10?.toFixed(2)}</span><span>MA20 {ma20?.toFixed(2)}</span><em>相对量 {current.volume ? (current.volume / (data.slice(-20).reduce((sum, candle) => sum + candle.volume, 0) / Math.min(20, data.length))).toFixed(2) : "0"}×</em></div><CandleChart key={`${market}-${stock.code}-${stock.initialVisibleCount || INITIAL_BARS}`} data={data} markers={tradeMarkers} market={market} /></div>
      <aside className="trade-panel"><div className="decision-head"><span>股票交易</span><small>仅市价委托 · 无固定期限</small></div><div className="horizon-track open-ended"><i /><span>已推进 {advancedDays} 个交易日 · 可随时结束</span></div><div className="price-block"><small>归一化价格</small><strong>{current.close.toFixed(2)}</strong><span className={dayChange >= 0 ? "up" : "down"}>{dayChange >= 0 ? "+" : ""}{dayChange.toFixed(2)}%</span></div>
        {shares > .000001 ? <div className="position-card"><div><small>持仓数量</small><b>{shareNf.format(shares)} 股</b></div><div><small>当前市值</small><b>{currencySymbol}{nf.format(positionValue)}</b></div><div className="position-bar"><i style={{ width: `${Math.min(100, positionValue / equity * 100)}%` }} /></div><span>仓位 {((positionValue / equity) * 100).toFixed(1)}%</span></div> : <div className="position-empty"><span className="empty-ring" /><b>当前空仓</b><small>不操作也是一种有效决策</small></div>}
        {!finished ? <><div className="mode-tabs"><button className={mode === "buy" ? "active buy" : ""} onClick={() => { setMode("buy"); setOrderInputMode("allocation"); setQuantityInput(""); }}>买入</button><button className={mode === "sell" ? "active sell" : ""} onClick={() => { setMode("sell"); setOrderInputMode("allocation"); setQuantityInput(""); }} disabled={!shares}>卖出</button></div><div className="order-type-row"><span>市价委托</span><small>下一交易日开盘成交</small></div><label className="field-label">{mode === "buy" ? "使用可用现金" : "卖出当前持仓"}</label><div className="allocation-grid" role="group" aria-label="选择委托仓位">{ORDER_ALLOCATIONS.map((value) => <button key={value} className={orderInputMode === "allocation" && allocation === value ? "selected" : ""} onClick={() => { setAllocation(value); setOrderInputMode("allocation"); setQuantityInput(""); }}>{value === 1 ? "全仓" : value === 1 / 3 ? "1/3" : value === .25 ? "1/4" : value === .5 ? "1/2" : "3/4"}</button>)}</div><label className="field-label quantity-label" htmlFor="order-quantity"><span>或按股数委托</span><small>{market === "cn" ? "A股 100 股起，按整手交易" : "美股按整数股交易"}</small></label><div className={`quantity-field ${orderInputMode === "quantity" ? "active" : ""} ${quantityError ? "invalid" : ""}`}><button type="button" className="quantity-step" aria-label={`减少 ${lotSize} 股`} disabled={isRevealing || !quantityInput || Number(quantityInput) <= 0} onClick={() => adjustQuantity(-1)}>−</button><input id="order-quantity" inputMode="numeric" pattern="[0-9]*" placeholder={market === "cn" ? "输入股数，如 500" : "输入股数，如 25"} value={quantityInput} onFocus={() => setOrderInputMode("quantity")} onChange={(event) => { setOrderInputMode("quantity"); setQuantityInput(event.target.value.replace(/\D/g, "").slice(0, 7)); }} /><button type="button" className="quantity-step" aria-label={`增加 ${lotSize} 股`} disabled={isRevealing || maxQuotedQuantity <= 0 || Number(quantityInput || 0) >= maxQuotedQuantity} onClick={() => adjustQuantity(1)}>＋</button><span>股</span></div>{quantityError ? <p className="order-error">{quantityError}</p> : <div className="order-estimate"><span>预计委托 <b>{shareNf.format(estimatedQuantity)} 股</b></span><small>按当前价约 {currencySymbol}{nf.format(estimatedOrderValue)} · 实际以次日开盘价为准</small></div>}<div className="field-label holding-label">成交后推进多久</div><div className="duration-grid" role="group" aria-label="选择持有交易日数">{([1, 3, 5] as const).map((value) => <button key={value} className={revealDays === value ? "selected" : ""} onClick={() => setRevealDays(value)} disabled={value > remainingDays}>{value} 天</button>)}</div><button className={`primary-action ${mode}`} disabled={tradeDisabled} onClick={() => advance("trade")}>{isRevealing ? "行情逐日推进中…" : `市价${mode === "buy" ? "买入" : "卖出"} ${shareNf.format(estimatedQuantity)} 股 · 推进 ${Math.min(revealDays, remainingDays)} 天`} {!isRevealing && <span>→</span>}</button><button className="hold-action" disabled={isRevealing || remainingDays <= 0} onClick={() => advance("hold")}>{isRevealing ? "逐根加载真实行情" : `${shares ? "保持仓位" : "保持空仓"} ${Math.min(revealDays, remainingDays)} 天`}</button><button className="finish-action" onClick={finishGame}>提前结束并揭晓股票</button><p className="hint">不支持限价、做空或融资；没有固定天数限制，可一直决策到该段真实历史结束</p></> : <div className="finished-panel"><small>本局已结束</small><b>{profile.title}</b><button className="primary-action" onClick={() => setResultOpen(true)}>查看完整结算</button></div>}
      </aside>
    </section><footer className="source-note">A股 {initialChallenges.cn.universeSize.toLocaleString("zh-CN")} 只全市场股票池 · 美股 {initialChallenges.us.universeSize} 只 · 每局按需加载真实日线 · 不构成投资建议</footer>

    {rulesOpen && <dialog open className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setRulesOpen(false); }}><section className="rules-modal"><button className="modal-close" onClick={() => setRulesOpen(false)}>×</button><small>HOW TO PLAY</small><h2>在真实牛熊周期里做决策</h2><ol><li><b>市场</b><span>可随时选择 A 股或美股；A 股从 {initialChallenges.cn.universeSize.toLocaleString("zh-CN")} 只全市场股票池中抽取。</span></li><li><b>观察</b><span>开局提供 120 根真实日 K，覆盖约半年走势，股票身份和历史日期隐藏。</span></li><li><b>委托</b><span>仅支持市价买卖，可选 1/4、1/3、1/2、3/4、全仓，也可直接输入股数；统一在下一交易日开盘成交。</span></li><li><b>交易单位</b><span>A 股买卖按 100 股整手处理，美股按整数股处理；不允许超出可用资金或当前持仓。</span></li><li><b>持有</b><span>每次选择向前推进 1、3 或 5 个真实交易日，期间仓位不变、净值逐日计算。</span></li><li><b>周期</b><span>不设固定交易日上限；你可以持续决策到该段真实历史结束，也可以随时提前结束并揭晓答案。</span></li><li><b>评分</b><span>综合超额收益、最大回撤和交易纪律；两类市场排行榜独立计算。</span></li><li><b>复盘</b><span>结算后揭晓真实股票、交易所和日期，并从仓位、风险、频率、持有周期和买卖时机生成深度画像。</span></li></ol><button className="primary-action" onClick={() => setRulesOpen(false)}>继续挑战</button></section></dialog>}
    ...
    {resultOpen && <div className="modal-backdrop result-backdrop"><section className="result-modal"><small className="eyebrow">{gameMode === "daily" ? `${marketLabel}今日盲盘 #${today.slice(5).replace("-", "")}` : `${marketLabel}随机练习`} · 股票揭晓</small><h1>{stock.name}</h1><p className="stock-code">{stock.market} · {stock.code}</p><div className={`result-hero ${returnRate >= 0 ? "positive" : "negative"}`}><span>最终收益</span><strong>{returnRate >= 0 ? "+" : ""}{returnRate.toFixed(2)}%</strong><small>{currencySymbol}{nf.format(INITIAL_CASH)} → {currencySymbol}{nf.format(equity)}</small></div><div className="result-grid"><div><small>操盘评分</small><b>{skillScore}</b></div><div><small>股票同期</small><b className={benchmark >= 0 ? "up" : "down"}>{benchmark >= 0 ? "+" : ""}{benchmark.toFixed(2)}%</b></div><div><small>超额收益</small><b>{excess >= 0 ? "+" : ""}{excess.toFixed(2)}%</b></div><div><small>最大回撤</small><b>{maxDrawdown.toFixed(2)}%</b></div></div>{gameMode === "daily" && <div className="rank-result">{scoreStatus === "loading" ? <p>正在由服务器复算决策路径并生成排名…</p> : scoreStatus === "error" ? <button onClick={() => setScoreStatus("idle")}>成绩提交失败，点击重试</button> : scoreboard?.playerScore ? <><div><small>{marketLabel}今日首次成绩</small><b>第 {scoreboard.playerScore.rank} 名</b><span>超过 {scoreboard.playerScore.percentile}% 玩家 · 共 {scoreboard.total} 人</span></div>{duelId && <div className="duel-result"><small>好友对决</small>{scoreboard.opponent ? <b className={scoreboard.playerScore.score >= scoreboard.opponent.score ? "win" : "lose"}>{scoreboard.playerScore.score} : {scoreboard.opponent.score}<em>{scoreboard.playerScore.score >= scoreboard.opponent.score ? "你领先" : "好友领先"}</em></b> : <span>好友尚未完成，稍后再来看</span>}</div>}</> : <p>完成校验后显示今日排名</p>}</div>}<div className="profile-card"><small>本局交易画像</small><b>{profile.title}</b><p>{profile.text}</p></div><button className="analysis-action" onClick={() => setAnalysisOpen(true)}><span><small>个性化交易复盘</small><b>深度分析我的画像</b></span><i>查看仓位、风险、择时与训练建议 →</i></button><div className="date-reveal">本局走到：{stock.candles[initialVisibleCount - 1].date} — {stock.candles[visibleCount - 1].date} · 完整数据：{stock.candles[0].date} — {stock.candles.at(-1)?.date}（{stock.candles.length.toLocaleString("zh-CN")} 根）· 共 {trades} 次交易</div><div className="result-actions three"><button className="primary-action" onClick={shareResult}>{shareStatus || (gameMode === "daily" ? "发起好友同图挑战" : "分享战绩")}</button><button className="hold-action" disabled={challengeLoading} onClick={() => void resetGame(gameMode === "daily" ? "practice" : "daily")}>{challengeLoading ? "正在抽取历史行情…" : gameMode === "daily" ? "随机练习" : "返回今日挑战"}</button><button className="review-action" onClick={() => setResultOpen(false)}>返回复盘 K 线</button></div></section></div>}

    {analysisOpen && <dialog open className="modal-backdrop analysis-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setAnalysisOpen(false); }}><section className="analysis-modal"><button className="modal-close" onClick={() => setAnalysisOpen(false)}>×</button><small className="eyebrow">MY TRADING DNA · 本局深度复盘</small><div className="analysis-hero"><span>你的交易画像</span><h2>{deepProfile.title}</h2><p>{deepProfile.summary}</p><small>{deepProfile.confidence}</small></div><div className="analysis-metrics">{deepProfile.metrics.map((metric) => <div key={metric.label}><small>{metric.label}</small><b>{metric.value}</b><span>{metric.note}</span></div>)}</div><div className="analysis-section-head"><b>四维诊断</b><span>基于本局逐日净值与真实成交点</span></div><div className="analysis-dimensions">{deepProfile.dimensions.map((dimension) => <article key={dimension.label} className={dimension.tone}><div><span>{dimension.label}</span><b>{dimension.grade}</b></div><p>{dimension.text}</p></article>)}</div><div className="analysis-section-head"><b>带走的 3 条经验</b><span>下一局可以直接执行</span></div><ol className="analysis-lessons">{deepProfile.lessons.map((lesson, index) => <li key={lesson}><i>0{index + 1}</i><span>{lesson}</span></li>)}</ol><div className="training-goal"><small>NEXT SESSION GOAL</small><b>{deepProfile.trainingGoal}</b></div><p className="analysis-disclaimer">画像只反映本局历史样本，用于训练决策与复盘，不构成投资建议。</p><button className="primary-action" onClick={() => setAnalysisOpen(false)}>返回结算</button></section></dialog>}
  </main>;
}
