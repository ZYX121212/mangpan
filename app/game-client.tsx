...
import { HORIZON_DAYS, INITIAL_BARS, INITIAL_CASH, ORDER_ALLOCATIONS, TOTAL_BARS, clamp, lotSizeFor, orderQuantity, type MarketKind, type OrderAllocation, type ReplayAction } from "./game-config";
...
export default function GameClient({ initialChallenges }: { initialChallenges: InitialChallenges }) {
  ...
  const advance = async (action: "trade" | "hold") => {
    ...
    if (action === "trade") {
      const amount = orderQuantity({ market, kind: mode, price: execution, cash, shares, allocation, quantity: requestedQuantity });
  ...
  const tradeDisabled = isRevealing || finished || estimatedQuantity <= 0 || Boolean(quantityError);
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      <aside className="trade-panel"><div className="decision-head"><span>股票交易</span><small>仅市价委托 · 共模拟 {HORIZON_DAYS} 日</small></div><div className="horizon-track"><i style={{ width: `${advancedDays / HORIZON_DAYS * 100}%` }} /><span>{advancedDays} / {HORIZON_DAYS} 交易日</span></div><div className="price-block"><small>归一化价格</small><strong>{current.close.toFixed(2)}</strong><span className={dayChange >= 0 ? "up" : "down"}>{dayChange >= 0 ? "+" : ""}{dayChange.toFixed(2)}%</span></div>
