...
function CandleChart({ data }: { data: Candle[] }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const start = Math.max(0, data.length - 82);
      ...
      if (hover != null && hover >= 0 && hover < shown.length) {
        ...
        const label = `第 ${start + hover + 1} 日  开 ${d.open.toFixed(2)}  高 ${d.high.toFixed(2)}  低 ${d.low.toFixed(2)}  收 ${d.close.toFixed(2)}`;
        ctx.font = "10px ui-monospace, SFMono-Regular, Menlo, monospace";
  ...
  return <canvas ref={canvasRef} className="chart-canvas" aria-label="真实历史日K线图" onMouseLeave={() => setHover(null)} onMouseMove={(event) => { const rect = event.currentTarget.getBoundingClientRect(); const shownLength = Math.min(82, data.length); const slot = (rect.width - 62) / shownLength; setHover(Math.max(0, Math.min(shownLength - 1, Math.floor(event.nativeEvent.offsetX / slot)))); }} />;
}
...
export default function Home() {
  ...
  const reset = () => {
    ...
    setVisibleCount(INITIAL_BARS); setRound(0); setCash(INITIAL_CASH); setShares(0); setMode("buy"); setAllocation(1); setTrades(0); setEquityHistory([INITIAL_CASH]); setEnded(false); setRevealPulse(0);
  };
  ...
  return (
    <main className="shell">
      ...
      <section className="workspace">
        <div className={`chart-panel ${revealPulse ? "revealed" : ""}`} key={`chart-${revealPulse}`}>
